...
function withValidator(callback) {
  ...
  if (!JAVA_HOME) {
    ...
    fs.exists('support/XMLValidator.class', function(exists) {
      if (!exists) {
        spawn(JAVA_HOME + '/bin/javac', [ 'support/XMLValidator.java' ]).on('exit', callback);
      } else {
        callback();
      }
    });
...
function validateXML(xml, schema, callback) {
  ...
  withValidator(function(err) {
    ...
    var validator = spawn(JAVA_HOME + '/bin/java', [ 'support.XMLValidator', '-stdin', '-schema=' + schema ]);
